Shader "UEngine/UE Blend EyeShadow"
{
    Properties
    {
        ...
        _blendArea ("BlendArea", Range(0, 1)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _saturation,_brightness,_glossiness;
            ...
            fixed4 frag (v2f i) : COLOR
            {
                ...
                _blendArea = _blendArea*2-1 ;
                half4 blusherTex = tex2D(_TattooTex, filter_uv(i.targetUV, _UVRegion));
                //half4 blusherTex = tex2D(_TattooTex, i.uv);
                half  blusherlerp = saturate(saturate(_blendArea+blusherTex.g)*blusherTex.r*_blendAlpha);
      
                float4 faceColor = tex2D(_MainTex, i.uv);
                
                faceColor.rgb = lerp(faceColor.rgb,((faceColor.rgb*.3+.7)*_blendColor.rgb), blusherlerp);

                //faceColor.a += (blusherlerp*_glossiness);
                //faceColor.a = (faceColor.a*.8+.2)*;
