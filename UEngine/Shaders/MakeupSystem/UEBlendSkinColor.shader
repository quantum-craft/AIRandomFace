Shader "UEngine/UE Blend SkinColor"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex;
            sampler2D _TattooTex,_uselesscol;
            ...
            {
                half glossiness = tex2D(_uselesscol,i.uv*20-float2(19,0)).r*_glossiness;//*20-float2(19,0)

                _blendColor.xyz = lerp(.5,_blendColor.xyz, _saturation );
                _blendColor.xyz =  saturate(_blendColor.xyz + _brightness*2-1);
                
                half4 blendTex = tex2D(_TattooTex, filter_uv(i.targetUV, _UVRegion));
                half blendLerp = blendTex.r;

                float4 srcColor = tex2D(_MainTex, i.uv);

                srcColor.a = glossiness+srcColor.a;

                srcColor.rgb = lerp(srcColor.rgb,srcColor.rgb*_blendColor.rgb, blendLerp);
                return srcColor;
            }
            ENDCG
        }
